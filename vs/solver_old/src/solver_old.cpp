...
namespace NTree {
    ...
    */
    struct LCA {
        ...
        // 初期化
        void init(const Graph& G, int root = 0) { // TODO: 木の中心を root にした方が速い？
            int V = G.size();
            int K = 1;
            while ((1 << K) < V) K++;
...
namespace NSteiner {
    ...
    void initialize(const Input& input) {
        ...
        tours = input.ts;
        xys = input.xys;
        using E = std::pair<int, int>; // (v, e)
        using Q = std::queue<int>;
        std::vector<std::vector<E>> G(N);
        for (int e = 0; e < input.M; e++) {
            const auto& [u, v] = uvs[e];
    ...
    struct Steiner {
        ...
        int term[N];
        bool used[N];
        std::pair<int, int> min[N];

        std::bitset<N * 3> B;
        int edge_cost;

        void run(const std::bitset<N>& term_bits, int initial_t) {
            //Perf perf(__FUNCTION__);
            ...
            used[initial_t] = true;
            for (int t = 0; t < T; t++) {
                min[t] /* T */ = { gds[term[initial_t]][term[t]], term[initial_t] }; // gds: NxN
            }
            while (true) {
                ...
                while (s != u) {
                    for (int k = 0; k < T; k++) {
                        int i = term[k];
                        if (!used[k]) {
                            chmin(min[k], { gds[u][i], u });
                        }
                    }
                    int v = gprvv[s][u], e = gprve[s][u];
                    if (!B[e]) {
                        B[e] = true;
                        edge_cost++;
                    }
        ...
        std::vector<std::vector<int>> to_graph() const {
            ...
            for (int i = 0; i < (int)tours.size(); i++) {
                assert(uf.same(t, tours[i]));
                t = tours[i];
            }
            return g;
        }

        int compute_tour_length() const {
            std::vector<int> to_value;
            {
                std::set<int> st;
                for (int e = 0; e < M; e++) {
                    if (B[e]) {
                ...
                to_value.assign(st.begin(), st.end());
            }
            std::array<int, N> to_index;
            to_index.fill(-1);
            for (int i = 0; i < (int)to_value.size(); i++) {
                to_index[to_value[i]] = i;
            }
            NLCA::LCA lca((int)to_value.size());
            for (int e = 0; e < M; e++) {
                if (B[e]) {
                    const auto& [u, v] = uvs[e];
                    lca.add_edge(to_index[u], to_index[v]);
                }
            }
            lca.build();
            int tour_length = 0, t = 0;
            for (int i = 0; i < (int)tours.size(); i++) {
                tour_length += lca.dist(to_index[t], to_index[tours[i]]);
                t = tours[i];
            }
            return tour_length;
        }

        NLCA::LCA to_lca() const {
            NLCA::LCA lca(N);
            for (int e = 0; e < M; e++) {
                if (B[e]) {
                    auto [u, v] = uvs[e];
        ...
        void show(int delay = 0) {
            ...
            std::vector<bool> to_visit(N);
            to_visit[0] = true;
            for (int t : tours) to_visit[t] = true;
            constexpr int margin = 50;
            constexpr int width = 1000;
            std::vector<cv::Point> points;
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char** argv) {
    ...
    std::vector<std::vector<int>> G5;
    {
        ...
        for (const auto& tree : trees.trees) {
            ...
            auto gf = tree.to_graph();
            int diam = NTree::tree_diameter(g);
            if (chmin(min_diam, diam)) {
                G4 = g;
            }
            NTree::LCA lca(gf, tree.root);
            int tour = 0, t = 0;
            for (int i = 0; i < (int)input.ts.size(); i++) {
                tour += lca.get_dist(t, input.ts[i]);
                t = input.ts[i];
            }
    ...
    {
        int min_tour = INT_MAX;
        NSteiner::initialize(input);
        NSteiner::Steiner s;
        std::bitset<N> term_bits;
        for (int t : critical_nodes) {
            term_bits[t] = true;
        }
        for (int i = 0; i < (int)critical_nodes.size(); i++) {
            s.run(term_bits, i);
            int tour = s.compute_tour_length();
            if (chmin(min_tour, tour)) {
                G6 = s.to_graph();
                dump(i, min_tour);
            }
    ...
    auto paths6 = compute_shortest_paths(G6);
    ...
    auto initial_A6 = compute_initial_A(input, G6);
    dump(initial_A.size(), initial_A2.size(), initial_A3.size(), initial_A4.size(), initial_A5.size(), initial_A6.size(), input.LA);
    ...
    auto tour6 = compute_tour(input, paths6);
    dump(tour.size(), tour2.size(), tour3.size(), tour4.size(), tour5.size(), tour6.size());
    ...
    {
        auto [score, A, ans] = solve(input, tour6, initial_A6);
        if (chmin(best_score, score)) {
            best_A = A;
            best_ans = ans;
            dump(best_score);
