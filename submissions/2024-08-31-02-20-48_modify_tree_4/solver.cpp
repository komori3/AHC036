...
std::tuple<int, std::vector<int>, std::vector<std::string>> solve(
    ...
    while (idx + 1 < (int)tour.size()) {
        ...
        for (int begin = 0; begin + NInput::LB <= (int)A.size(); begin++) {
            for (int i = begin; i < begin + NInput::LB; i++) {
                signal[A[i]]++;
            }
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char** argv) {
    ...
    while (timer.elapsed_ms() < 2000) {
        ...
        if (tour_length < ntour_length) {
            ...
            //if (ntour_length < tour_length) {
            //    dump(loop, ntour_length);
            //}
        ...
        if (loop % 1000 == 0) {
            ...
            auto [score, A, ans] = solve_opt_naive(tour, initial_A);
            if (chmin(best_score, score)) {
                best_g = g;
                dump(loop, score, A.size());
            }
        }
    }
