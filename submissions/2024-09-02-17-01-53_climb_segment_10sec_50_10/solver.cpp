...
struct SegmentModifier {
    ...
    SegmentModifier(const std::vector<int> tour_, const std::vector<int>& A_) : tour(tour_), A(A_) {
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char** argv) {
    ...
    auto A = compute_initial_A(g);
    {
        ...
        const int margin = NInput::LA - (int)A.size();
        const int nsegment = (margin + NInput::LB - 1) / NInput::LB;

        // (a_left, length, tour_left)
        std::vector<std::pair<int, int>> segments(nsegment);
        ...
        std::vector<int> tour_lefts(nsegment);

        auto assign_segment = [&](int segment_id, int tour_left) {
        ...
        int cost = compute_signal_cost(tour, A);
        int loop = 0;
        while (timer.elapsed_ms() < 10000) {
            ...
            int ncost = compute_signal_cost(tour, A);
            if (cost < ncost) {
